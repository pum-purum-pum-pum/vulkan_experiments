...
impl Context {
    pub fn new(event_pump: &EventPump, sdl_context: sdl2::Sdl) -> Context {
        ...
        let ubo_layout = Context::create_descriptor_set_layout(&device);
        let (graphics_pipeline, pipeline_layout) = Context::create_graphics_pipeline(
            &device,
            render_pass,
        ...
        let descriptor_pool =
            Context::create_descriptor_pool(&device, swapchain_stuff.swapchain_images.len());
        let descriptor_sets = Context::create_descriptor_sets(
            &device,
            descriptor_pool,
            ubo_layout,
            &uniform_buffers,
        ...
        let camera = Camera::new(WINDOW_WIDTH as f32, WINDOW_HEIGHT as f32);
        Context {
            // winit stuff
            window,
...
impl Context {
    ...
    fn update_uniform_buffer(&mut self, current_image: usize, _delta_time: f32) {
        ...
        // let (view, proj) = Context::create_camera(width as f32, height as f32);
    ...
    fn create_graphics_pipeline(
        ...
        let vert_shader_module = share::create_shader_module(
            device,
            include_bytes!("shaders/shader.vert.spv").to_vec(),
        );
        let frag_shader_module = share::create_shader_module(
            device,
            include_bytes!("shaders/shader.frag.spv").to_vec(),
        );
